    ...
    if (id < nSegs * dFeats)
    {
        ...
        for(int l = 0; l < nPixels; l++ )
        {
            if (d_SegsIdx[l] == idseg+1)
            {
                ...
                /*val = val + d_Feats[dFeats * l + idFea];*/
                count = count + 1;
            }
        }
...
std::vector<at::Tensor> SpixelAggr_avr_cuda_forward(
    ...
    int nPixels = Feat.size(1) * Feat.size(2);
    int dFeats = Feat.size(0);
    int nSegs = nPsPerSegs.size(0);

    /*at::Tensor Res = at::zeros(at::CUDA(at::kFloat),{nSegs, dFeats});*/

    /*at::Tensor res = at::zeros_out(at::CUDA(at::kFloat), {nSegs, dFeats}); ::CUDA(at::kFloat).*/

    /*printf("%d, %d, %d\n", nPixels, dFeats, nSegs);*/
...
std::vector<at::Tensor> SpixelAggr_avr_cuda_backward(
    ...
    /*auto res = at::ones_like(Feat);*/
    const int nSegs = dzdy.size(1);
    const int nPixels = res.size(1) * res.size(2);
    const int dFeats = res.size(0);
    ...
    AT_DISPATCH_FLOATING_TYPES(res.type(), "SpixelAggr_avr_cuda_back", ([&] {
