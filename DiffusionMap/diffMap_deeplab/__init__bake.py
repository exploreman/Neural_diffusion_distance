...
class BSDataSet(data.Dataset): #Berkeley segmentation dataset
    ...
    def __getitem__(self, index):
        ...
        #seglabel = torch.from_numpy(seglabel)
        neigMask = mat_file['neig_mask'] #.value.transpose([0, 2, 1])
        #neigMask = torch.from_numpy(neigMask)

        len = mat_file['gt_W'].__len__()
        randIdx = torch.LongTensor(1).random_(0, len)
        gt_Wei = mat_file['gt_W'][0][randIdx]
        #gt_Wei = torch.from_numpy(gt_Wei)
        ...
        #    image = image[:, :, ::flip]
        #    seglabel = seglabel[:, ::flip]

        image = np.asarray(image, np.float32)
        seglabel = np.asarray(seglabel, np.float32)
        neigMask = np.asarray(neigMask, np.float32)
        gt_Wei = 1 - np.asarray(gt_Wei, np.float32)


        return image.copy(), seglabel.copy(), neigMask.copy(), gt_Wei.copy()  #, size, name
...
class BSDataTestSet(data.Dataset):  #Berkeley segmentation dataset
    def __init__(self, root, list_path, crop_size=(505, 505), mean=(128, 128, 128)):
        ...
        # for split in ["train", "trainval", "val"]:
        for name in self.img_ids:
            img_file = osp.join(self.root, "%s.mat" % name)
    ...
    def __getitem__(self, index):
        datafiles = self.files[index]
        mat_file = h5py.File(datafiles["image"])

        #image = cv2.imread(datafiles["img"], cv2.IMREAD_COLOR)
        image = mat_file['image'].value.transpose([0, 2, 1])
        image = torch.from_numpy(image)
        seglabel = mat_file['label'].value.transpose([0, 2, 1])
        seglabel = torch.from_numpy(seglabel)
        neigMask = mat_file['neigMask'].value.transpose([0, 2, 1])
        neigMask = torch.from_numpy(neigMask)
        gt_Wei = mat_file['gt_W'].value.transpose([0, 2, 1])
        gt_Wei = torch.from_numpy(gt_Wei)
        ...
        return image, seglabel, neigMask, size
