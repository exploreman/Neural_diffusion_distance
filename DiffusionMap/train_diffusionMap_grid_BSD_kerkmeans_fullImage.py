...
from diffMap_deeplab import BSDataSet_nocrop
...
# parameters setting
BATCH_SIZE = 1
DATA_DIRECTORY = '/home/jiansun/Dataset/BSD500/ALL_new_orig/' #/home/jiansun/Dataset/VOC/VOC2012/VOCdevkit/VOC2012/
DATA_LIST_PATH =   './diffMap_deeplab/data/trainval.txt' #'./diffMap_deeplab/data/list_all.txt' #list_msra.txtlist_all.txt list_msra val_single.txt' #list_all './diffMap_deeplab/data/val_single.txt' # './diffMap_deeplab/data/trainval.txt' #'./diffMap_deeplab/data/val_single.txt' #
IGNORE_LABEL = 255
INPUT_SIZE = '321,321'
LEARNING_RATE = 0.8e-6 #1e-6 #4e-6
MOMENTUM = 0.9
NUM_CLASSES = 21
NUM_STEPS = 200000
POWER = 0.9
RANDOM_SEED = 1234
RESTORE_FROM =  './snapshots_diffMap_grid/BSD_diffMap120000_clus_BSD500_trainval.pth' #BSD_diffMap60000_clus_BSD500_v7.pth' #'./dataset/MS_DeepLab_resnet_pretrained_COCO_init.pth'  #'./snapshots_diffMap_grid/BSD_diffMap126000_clus_BSD500.pth' #'./snapshots_diffMap_grid/BSD_diffMap84000_clus_BSD500_v6.pth' #'./snapshots_diffMap_grid/BSD_diffMap36000_clus_BSD500_v4.pth' # './dataset/MS_DeepLab_resnet_pretrained_COCO_init.pth' # './snapshots_diffMap_grid/BSD_diffMap117000_clus_BSD500.pth' ##378000 #288'./dataset/MS_DeepLab_resnet_pretrained_COCO_init.pth' #
...
def main():
    ...
    if 0:
        ...
        st = 120000 #192000#90000#190000 #156 #160
        #model.layer6._modules['0'].dist2SimiMatrix1._parameters['weights'].data = torch.tensor(-8.0)
        #model.layer6._modules['0'].compDiffDist._parameters['weights'] = torch.nn.Parameter(torch.tensor(2.0))
...
#    torch.backends.cudnn.benchmark = True
    ...
    trainloader = data.DataLoader(BSDataSet_nocrop(args.data_dir, args.data_list, max_iters=args.num_steps*args.batch_size, mirror=True,
                                            mean=IMG_MEAN, scale=False), batch_size=args.batch_size, shuffle=False, num_workers=1, pin_memory=True)
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        images, gt_Ws, gt_clusters, gt_lst = batch
        ...
        # loss_km = loss_calc_mask(pred, gt_Ws, mask)

        if iter > 120000 - 1:
            #loss_cluster = loss_kerkmeans_ss(pred, gt_clusters)
            loss_hr = loss_calc_hiReso(pred, gt_clusters, gt_lst, feat_hirc)
            loss = loss_km + lamb * loss_hr
        else:
            loss = loss_km
        ...
        if iter >= args.num_steps-1:
            print ('save model ...')
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'BSD_diffMap'+str(args.num_steps)+'_clus_BSD500_trainval_full.pth')) #_trainval
            break

        if iter % args.save_pred_every == 0 and i_iter!=0:
            print ('taking snapshot ...')
            torch.save(model.state_dict(),osp.join(args.snapshot_dir, 'BSD_diffMap'+str(iter)+'_clus_BSD500_trainval_full.pth')) #_trainval
...
plt.show()
plt.imshow(pred[id, 10 * 41 + 35, :].reshape(41,41).detach())
plt.show()
'''
